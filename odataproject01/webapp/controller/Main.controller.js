sap.ui.define([
	...
	function (Controller, MessageToast, ODataModel) {
		...
		return Controller.extend("com.ui.odataproject01.controller.Main", {
			onInit: function () {
				...
				loModelService.read("/Categories", {
					success: function(oData) {
						...
						new Array("mainLblTextService").map(
							(id, index) => {
								that.byId(id).setText(oData.results[index].Name)
						})
					},

					error: function(error){
						this.getView().byId("mainLblTextService").setText("");
					}
				}, this);
